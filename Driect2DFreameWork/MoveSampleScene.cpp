...
void CMoveSampleScene::Update()
{
	...
	{
		CTransform& cTrnasform = m_pEagleObject->GetTransform();
		SVector2 vPos = m_pEagleObject->GetTransform().GetTranslate();
		SVector2 vTargetPos = m_pPlayerObject->GetTransform().GetTranslate();
	...
	m_pCherryObject->GetTransform().SetTranslate(0, 50);

	m_pGemObject->Update();
	m_pGemObject->GetTransform().SetTranslate(0, 80);

	m_pItemEffectObject->Update();
	m_pItemEffectObject->GetTransform().SetTranslate(0, 100);

	m_pDeathEffectObject->Update();
	m_pDeathEffectObject->GetTransform().SetTranslate(0, 120);
}
